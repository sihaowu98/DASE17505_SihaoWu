...
class PID_ctrl:
    ...
    def __update(self, stamped_error):
        ...
        for i in range(1, len(self.history)):
            ...
            error_dot+=(self.history[i][0] - self.history[i-1][0])/dt # 误差变化率
            
            
        error_dot/=len(self.history) # 平均误差变化率
        dt_avg/=len(self.history) # 平均时间间隔
        
        
        sum_=0
        for hist in self.history:
            sum_+=hist[0] # 累加所有历史误差值
        
        
        error_int=sum_*dt_avg # 积分项 = 误差累加和 × 平均时间间隔
